...
public:
	...
	*********************************************************************************************/
	bool drop(const T &element) {
		...
		while (N != last) {
			...
			N = N->getNext(); //Move to the next node (5)
		}
		return false;
	};
	...
	*********************************************************************************************/
	void print(void) {
		Node *N = first;  //Unitialised variable (1)
		cout << N->getValue() << endl; //Allow it to print single node lists (4)

		while (N != last) {
			N = N->getNext();  //Not incrementing to next node (3)
...
int main(int argc, char* argv[]) {
	...
	DoubleLinkedList<string>* L = new DoubleLinkedList<string>;  //Uninitialised variable (2)
	...
	L->print();		//All good
					//add the last number
	L->append(s6);  //put in the right spot (6)
	L->print();		//Done!
